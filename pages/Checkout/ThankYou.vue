...
export default {
  ...
  mounted () { 
     window.analytics.track("Order Completed", {
        ...
        revenue:parseInt(cartGetters.getTotals(this.cart).total,10),
        products:this.products.map(item => {return {product_id: item.productId,
        price:parseInt((item.price.value.centAmount)/100,10),variant:''}})});
  },
  setup(props, context) {
    const { cart } = useCart();
