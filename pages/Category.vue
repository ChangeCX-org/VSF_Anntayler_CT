        ...
        <div class="products" v-if="!loading">
          ...
          >
            <SfProductCard
              ...
              @click:wishlist="isInWishlist({ product }) ? wishlistremove({ product }) : wishlistadd({ product })"
              @click:add-to-cart="productaddcart({ product, quantity: 1 })"
            />
          </transition-group>
          <transition-group
            v-else
          ...
          >
            <SfProductCardHorizontal
              ...
              @click:wishlist="isInWishlist({ product }) ? wishlistremove({ product }) : addItemToWishlist({ product })"
              @click:add-to-cart="productaddcart({ product, quantity: 1 })"
              :link="localePath(`/p/${productGetters.getId(product)}/${productGetters.getSlug(product)}`)"
            >
...
export default {
  ...
  setup(props, context) {
    ...
    const { addItem: addItemToWishlist, removeItem: removeItemFromWishlist, isInWishlist, wishlist } = useWishlist();
  ...
  "image_url": "",
  "name": product._name,
  "position": parseInt(0,10),
  "price": parseInt((product.price.value.centAmount)/100,10),
  ...
  "variant": ""
    ...
    const wishlistadd =(product)=>{
      ...
      window.analytics.track("Product Added to Wishlist", {
        ...
        "position": parseInt(0,10),
        "price": parseInt((product.product.price.value.centAmount)/100,10),
        "product_id": product.product._id,
        "quantity": parseInt(0,10),
        "sku": product.product.sku,
        "url": product.product._slug,
        "variant": "",
        "wishlist_id": product.product._id.toString(),
        "wishlist_name": ""
      })
    };

    const wishlistremove = ({ product }) => {
      const wishlistProduct = wishlist.value.lineItems.find(lineItem => lineItem.name === product._name)
      removeItemFromWishlist({ product: wishlistProduct })
    }
    const applyFilters = () => {
      toggleFilterSidebar();
      changeFilters(selectedFilters.value);
